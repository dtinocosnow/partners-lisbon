# Snowflake AI Data Cloud - Sessao de Capacitacao para Partners

---

**Local:** Lisboa, Portugal
**Duracao:** 3 horas 


---

## Objetivo da Sessao

Proporcionar aos nossos partners uma visao completa e pratica da plataforma Snowflake AI Data Cloud, permitindo-lhes:

- Compreender a arquitetura e os pilares fundamentais da plataforma
- Observar as principais funcionalidades em acao atraves de demonstracoes ao vivo
- Explorar as capacidades de IA e aplicacoes nativas
- Realizar exercicios praticos guiados para consolidar os conhecimentos
- Definir um percurso claro de capacitacao e proximos passos

**Publico-alvo:** Data Analysts, Data Engineers, Architects e outros perfis tecnicos

---

## Agenda

| Horario | Duracao | Sessao | Orador |
|---------|---------|--------|--------|
| 09:00 - 09:10 | 10 min | **Boas-vindas e Objetivos** | Pedro |
| | | Apresentacoes, contexto do mercado portugues e objetivos da sessao | |
| 09:10 - 09:40 | 30 min | **Visao Geral do Snowflake AI Data Cloud** | Pedro |
| | | Arquitetura (armazenamento, computacao, governanca, colaboracao), capacidades-chave da plataforma, posicionamento | |
| 09:40 - 10:15 | 35 min | **Aprofundamento I: Fundamentos da Plataforma** | David |
| | | Snowsight, seguranca e governanca (RBAC, masking policies, row access), alavancas de desempenho e custo, ingestao de dados (Snowpipe, Dynamic Tables). **Demo ao vivo** | |
| 10:15 - 10:25 | 10 min | **Intervalo** | |
| 10:25 - 10:55 | 30 min | **Aprofundamento II: IA e Aplicacoes** | David |
| | | Cortex AI (Analyst, Search, Agents, ML Functions), interoperabilidade aberta (Iceberg, Polaris, OpenFlow), aplicacoes e marketplace interno. **Demo ao vivo** | |
| 10:55 - 11:25 | 30 min | **Laboratorio Pratico Guiado** | David |
| | | Exercicio acompanhado: criar base de dados, carregar dados, transformar, aplicar governanca e utilizar uma funcao de IA Cortex | |
| 11:25 - 11:30 | 5 min | **Pausa** | |
| 11:30 - 12:00 | 30 min | **Conclusao e Percurso de Capacitacao** | Pedro + David |
| | | Recursos de aprendizagem, Snowflake University, certificacoes, proximos passos concretos para os proximos 30-60 dias, definicao de acoes GTM conjuntas | |

---

## O Que Vao Levar Desta Sessao

1. **Compreensao clara** da arquitetura e proposta de valor do Snowflake
2. **Experiencia pratica** com a plataforma atraves do laboratorio guiado
3. **Visao das capacidades de IA** integradas nativamente na plataforma
4. **Roteiro de capacitacao** personalizado para cada perfil tecnico
5. **Acoes concretas** para os proximos 30-60 dias

---

## Pre-requisitos

- Computador portatil com acesso a internet e browser atualizado (Chrome, Firefox ou Edge)
- Conta de trial Snowflake (instrucoes de criacao serao fornecidas)
- Conhecimentos basicos de SQL (recomendado)

---

## Oradores

**Pedro** - Snowflake
Responsavel pela visao estrategica da plataforma e contexto de mercado.

**David** - Snowflake
Responsavel pelas demonstracoes tecnicas e laboratorio pratico.

---

## Recursos Uteis

- Criar conta trial: https://signup.snowflake.com
- Documentacao: https://docs.snowflake.com
- Guias de inicio rapido: https://quickstarts.snowflake.com
- Snowflake University: https://learn.snowflake.com
- Comunidade: https://community.snowflake.com

---

*Snowflake AI Data Cloud - A plataforma que unifica dados, IA e aplicacoes*

**Cores da marca:** Snowflake Blue #29B5E8 | Mid Blue #11567F
**Tipo de letra:** Texta (titulos) | Lato (corpo de texto)
